...
def get_embeddings(text_list, model, tokenizer, max_length=512):
    ...
    for text in text_list:
        ...
        if len(inputs['input_ids'][0]) < max_length:
            ...
            input_ids_chunks = [inputs['input_ids'][0][i:i + max_length] for i in range(0, len(inputs['input_ids'][0]), max_length)]
            attention_mask_chunks = [inputs['attention_mask'][0][i:i + max_length] for i in range(0, len(inputs['attention_mask'][0]), max_length)]
            chunk_embeddings = []
...
def main():
    ...
    embeddings1 = get_embeddings(df['pdt_inclexcl_ENG_CONTENT'].tolist(), model, tokenizer)
    print("Generating embeddings for 'pdt_product_detail_PRODUCTDESCRIPTION'...")
    embeddings2 = get_embeddings(df['pdt_product_detail_PRODUCTDESCRIPTION'].tolist(), model, tokenizer)
